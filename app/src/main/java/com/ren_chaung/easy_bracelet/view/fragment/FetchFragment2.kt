...
class FetchFragment2(private val addrIndex: Int) : BaseFragment(), FetchCallback {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_fetch, container, false).apply {
            BraceletManager2.fetchMultiBracelet(addrIndex, BraceletMachineManager.fetchNum, this@FetchFragment2)
        }
    }
        ...
        context?.let {
            ResultTipsDialog.Builder(it, ResultTipsDialog.Builder.State.FAIL, "无响应，请重试").create().apply {
                ...
                buttonDone.setOnSingleClickListener {
                    dismiss()
                    BraceletManager2.fetchBracelet(addrIndex, this@FetchFragment2)
                }
            }.show()
        }
    }
